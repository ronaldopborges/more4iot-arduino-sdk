...
public:
  ...
  bool newDataObject(const char *token, int dataFieldsAmount, double lon = 0.0, double lat = 0.0)
  {
    ...
    dataHeader.clear();
    dataFields.clear();
    dataHeader.push_back(DataAttribute("uuid", token));
...
class More4iotMqtt : public More4iot, public DataObjectImpl
{
private:
  ...
  String topicREceiver = "input";
public:
  inline More4iotMqtt(Client &client, const char *host, int port = 1883) : mqtt_client(client), host(host), port(port) {
  ...
  bool connect(const char *host, int port)
  {
    ...
    Serial.println("connecting mqtt host...");
    mqtt_client.setServer(host, port);
    return mqtt_client.connect(topic);
  }
  ...
  bool send()
  {
    ...
    mqtt_client.publish(topic, data.c_str());
